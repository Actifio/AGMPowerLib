...
[switch][alias("g")]$guided) 
{
    ...
    if ( (!($appname)) -and (!($imagename)) -and (!($imageid)) -and (!($appid)) )
    {
        ...
        write-host "Fetching VM list from AGM"
    ...
    if ($guided)
    {
        ...
        [string]$sharedvpcprojectid = Read-Host "Shared VPC projectid"
    ...
    if ($guided)
    {
        ...
        Write-Host ""
        Write-Host -nonewline "New-AGMLibGCPVM -imageid $imageid -vmname `"$vmname`" -cpu $cpu -memory $memory -ostype `"$OSType`" -gcpkeyfile `"$gcpkeyfile`" -volumetype `"$volumetype`" -tags `"$tags`" -projectid `"$projectid`" -sharedvpcprojectid `"$sharedvpcprojectid`" -regioncode `"$regioncode`" -zone `"$zone`" -networkid `"$networkid`" -subnetid `"$subnetid`" -privateipaddresses `"$privateipaddresses`" -bootdisksize $bootdisksize"
