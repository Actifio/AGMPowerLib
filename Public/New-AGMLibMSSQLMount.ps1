Function New-AGMLibMSSQLMount ([int]$appid,[int]$targethostid,[string]$imagename,[string]$targethostname,[string]$appname,[string]$sqlinstance,[string]$dbname,[string]$recoverypoint,[string]$recoverymodel,[string]$overwrite,[string]$label,[string]$consistencygroupname,[string]$dbnamelist,[string]$dbnameprefix,[string]$dbnamesuffix,[string]$recoverdb,[string]$userlogins,[string]$mountmode,[string]$mapdiskstoallesxhosts,[string]$mountpointperimage,[switch][alias("g")]$guided,[switch][alias("m")]$monitor,[switch][alias("w")]$wait) 
{
    <#
    .SYNOPSIS
    ...
    if ($guided)
    {
       ...
       Clear-Host  
        ...
        {   
            if ($recoverypoint)
            {
                Write-Host -nonewline "New-AGMLibMSSQLMount -imagename $imagename -label `"$label`" -targethostid $hostid -sqlinstance `"$sqlinstance`" -dbname `"$dbname`" -recoverdb $recoverdb -userlogins $userlogins -recoverymodel `"$recoverymodel`" -overwrite `"$overwrite`" -recoverypoint `"$recoverypoint`""
                if ($mountmode)
                {
            ...
            {
                Write-Host -nonewline "New-AGMLibMSSQLMount -imagename $imagename -label `"$label`" -targethostid $hostid -sqlinstance `"$sqlinstance`" -dbname `"$dbname`" -recoverdb $recoverdb -userlogins $userlogins -recoverymodel `"$recoverymodel`" -overwrite `"$overwrite`""
                if ($mountmode)
                {
                    Write-Host -nonewline " -mountmode $mountmode -mapdiskstoallesxhosts $mapdiskstoallesxhosts"
                }
        ...
        {   
            if ($recoverypoint)
            {
                Write-Host -nonewline "New-AGMLibMSSQLMount -imagename $imagename -label `"$label`" -targethostid $hostid -sqlinstance `"$sqlinstance`" -dbnamelist `"$dbnamelist`" -dbname `"$dbname`" -recoverdb $recoverdb -userlogins $userlogins -recoverymodel `"$recoverymodel`" -overwrite `"$overwrite`" -recoverypoint `"$recoverypoint`""
                if ($mountmode)
                {
                    Write-Host -nonewline " -mountmode $mountmode -mapdiskstoallesxhosts $mapdiskstoallesxhosts"
                }
            ...
            {
                Write-Host -nonewline "New-AGMLibMSSQLMount -imagename $imagename -label `"$label`" -targethostid $hostid -sqlinstance `"$sqlinstance`" -dbnamelist `"$dbnamelist`" -dbname `"$dbname`" -recoverdb $recoverdb -userlogins $userlogins -recoverymodel `"$recoverymodel`" -overwrite `"$overwrite`""
                if ($mountmode)
                {
                    Write-Host -nonewline " -mountmode $mountmode -mapdiskstoallesxhosts $mapdiskstoallesxhosts"
                }
        ...
        else 
        {
            if ($recoverypoint)
            {
                Write-Host -nonewline "New-AGMLibMSSQLMount -imagename $imagename -label `"$label`" -targethostid $hostid -sqlinstance `"$sqlinstance`" -consistencygroupname `"$consistencygroupname`" -dbnamelist `"$dbnamelist`" -recoverdb $recoverdb -userlogins $userlogins -recoverymodel `"$recoverymodel`" -overwrite `"$overwrite`" -recoverypoint `"$recoverypoint`""
                if ($dbnameprefix)
                {
            ...
            {
                Write-Host -nonewline "New-AGMLibMSSQLMount -imagename $imagename -label `"$label`" -targethostid $hostid -sqlinstance `"$sqlinstance`" -consistencygroupname `"$consistencygroupname`" -dbnamelist `"$dbnamelist`" -recoverdb $recoverdb -userlogins $userlogins -recoverymodel `"$recoverymodel`" -overwrite `"$overwrite`""
                if ($dbnameprefix)
                {
                    Write-Host -nonewline "-dbnameprefix `"$dbnameprefix`""
                }
